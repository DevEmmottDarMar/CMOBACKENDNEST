  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  afterInit(wss: WebSocketServerInterface) {
    this.logger.log(
      '🚀 WebSocket Gateway Inicializado (WS puro) - Railway Mode',
    );
      ...
      try {
        ...
        ws.once('message', async (data) => {
          try {
            ...
            this.logger.error(
              '❌ Error procesando mensaje de autenticación:',
              error,
            );
            ws.close();
          }
  ...
  private async authenticateClient(ws: WebSocket, request: any, token: string) {
    try {
      ...
      const clientId =
        request.headers['sec-websocket-key'] || Date.now().toString();
      ...
      ws.on('close', () => {
        ...
        this.emitConnectedUsers();
        this.logger.log(
          `👋 Cliente desconectado: ${user.nombre} (${userRole})`,
        );
      });

      ws.on('error', (error: Error) => {
        this.logger.error(
          `💥 Error en cliente WS ${clientId}: ${error.message}`,
        );
      });
    } catch (error) {
  ...
  handleMessage(client: WebSocket, @MessageBody() data: any): void {
    this.logger.log(
      `📨 Mensaje recibido de cliente WS (${client.readyState === WebSocket.OPEN ? 'abierto' : 'cerrado'}): ${JSON.stringify(data)}`,
    );
